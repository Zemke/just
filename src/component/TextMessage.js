...
export default React.memo(({body}) => {
  ...
  const isHeartEmoji = message => {
    const trimmed = message.trim();
    return /^\u2764\uFE0F$/.test(message) || /^\u2665\uFE0F$/.test(message);
  }
  ...
  return (
    <>
      <div className={'textMessage' + (isOnlyEmoji(body) ? ' onlyEmoji' : '') + (isHeartEmoji(body.trim()) ? ' heartEmoji' : '')}
      ...
      <div className="previews">
        {previews.filter(p => p.url || p.img).map((preview, idx) => (
          <div key={idx}
        ...
        {!!previews.filter(p => p.url || p.img).find(p => !!p.guteUrl) && (
